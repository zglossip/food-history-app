...
export const useRecipeCardService = (
  ...
  const { servingAmount, servingName, cuisineTypes, courseTypes, tags } =
    toRefs<Recipe>(recipe.value);
  ...
  const formatTag = (
    ...
    computed(() => {
      ...
      return `${tagName}s: ${tags.value.reduce(
  ...
  const formattedCuisineTag: ComputedRef<string | boolean> = formatTag(
    "Cusine",
    cuisineTypes
  );
  const formattedCourseTag: ComputedRef<string | boolean> = formatTag(
    "Course",
    courseTypes
  );
